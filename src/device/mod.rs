...
impl IpHub {
    pub fn new() -> IpHub {
        let (iface_sender, iface_receiver) = mpsc::unbounded_channel();
        let task = IpHubTask {
            iface_receiver,
            ifaces: Vec::new(),
        };
        ...
        S: IpSinkStream,
    {
        ...
        self.iface_sender.send(iface).unwrap();
    }
}
...
impl IpHubTask {
    ...
    fn start_send_outgoing(&mut self, recv_index: usize, packet: IpPacket) {
    ...
    fn poll_next_incoming(&mut self, cx: &mut task::Context) -> Poll<(usize, IpPacket)> {
        let mut index = 0;
        loop {
            let iface = match self.ifaces.get_mut(index) {
                Some(iface) => iface,
    ...
    fn poll_inner(&mut self, cx: &mut task::Context) -> Poll<()> {
        loop {
            match self.iface_receiver.poll_recv(cx) {
